...
class StatementFunction(Protocol):
    def __call__(self, region: str, account_id: str) -> list[IamStatement]: ...


# StatementFunction = Callable[[str, str], IamStatement]
...
class Stmt:
    ...
    @staticmethod
    def base_agentcore(agent_name: str) -> StatementFunction:
        """Base statements required for AgentCore runtime"""

        def stmt(region: str, account_id: str) -> list[IamStatement]:
    ...
    @staticmethod
    def agent_core_assume_role(agent_name: str) -> StatementFunction:
        def stmt(region: str, account_id: str) -> IamStatement:
            return [
                {
                    "Version": "2012-10-17",
